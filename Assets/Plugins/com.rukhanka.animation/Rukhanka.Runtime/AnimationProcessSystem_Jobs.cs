...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	)
	{
		...
		newRootMotionState[animationIndex] = new RootMotionAnimationStateComponent() { uniqueMotionId = atp.motionId, animationState = bonePose };
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void Execute()
	{
		//	This is relatively long operation so it is moved to separate job.
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
	{
		...
		while (cee.NextEntityIndex(out var i))
		{
			...
			flagsHolder.Resize(rigBones.Length);
			flagsHolder.Clear();

			var localBoneTransformsForRig = localBoneTransforms.GetSpan(boneDataOffset.x, rigBones.Length);
			var worldBoneTransformsForRig = worldBoneTransforms.GetSpan(boneDataOffset.x, rigBones.Length);
			var boneFlags = AnimationTransformFlags.CreateFromBufferRO(boneTransformFlags, boneDataOffset.y, rigBones.Length);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void Execute(Entity e, in DynamicBuffer<AnimationToProcessComponent> atp, in DynamicBuffer<PreviousProcessedAnimationComponent> ppa, ref DynamicBuffer<AnimationEventComponent> aec)
	{
		...
		Assert.IsTrue(ppaArr.Length <= UnsafeUtility.SizeOf<ulong>());
	#if RUKHANKA_DEBUG_INFO
		if (ppaArr.Length > UnsafeUtility.SizeOf<ulong>())
			Debug.LogError("Too many simultaneous animations! Change bits holder to the NativeBitArray if this error occurs.");
	#endif
		
		for (var i = 0; i < atpArr.Length; ++i)
		{
			var a = atpArr[i];
			var curTime = math.frac(a.time);
			var prevBufferId = GetPreviousBufferAnimationIndex(a.motionId, i, ppaArr);
			var prevTime = -0.001f;
			if (prevBufferId >= 0)
			{
				prevAnimationsProcessedIndices |= 1ul << prevBufferId;
				prevTime = math.frac(ppaArr[prevBufferId].animationTime);
				if (prevTime > curTime)
					prevTime -= 1.0f;
			}

			ref var aes = ref a.animation.Value.events;
			ProcessEventsForAnimation(e, ref aec, ref aes, new float2(prevTime, curTime));
		}
		...
		for (var i = 0; i < ppaArr.Length; ++i)
		{
			...
			var p = ppaArr[i];
			ref var events = ref p.animation.Value.events;
			var timeSpan = new float2(p.animationTime, 1.0f);
			ProcessEventsForEndedAnimation(e, ref aec, ref events, timeSpan);
		}
	}

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	void ProcessEventsForEndedAnimation(Entity e, ref DynamicBuffer<AnimationEventComponent> outEvents, ref BlobArray<AnimationEventBlob> events, float2 timeSpan)
	{
		var outEventsLastIndex = outEvents.Length;
		for (var i = events.Length - 1; i >= 0 && timeSpan.x < events[i].time; --i)
		{
			ref var ae = ref events[i];
			var evt = new AnimationEventComponent(ref ae);
			
			//	Use insert here to preserve of events ordering
			outEvents.Insert(outEventsLastIndex, evt);
			
		#if RUKHANKA_DEBUG_INFO
			if (doDebugLogging)
				Debug.Log($"Emit animation event for {e}. Name: {ae.name.ToFixedString()}, F: {ae.floatParam}, I: {ae.intParam}, S: {ae.stringParam.ToFixedString()}");
		#endif
		}
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void ProcessEventsForAnimation(Entity e, ref DynamicBuffer<AnimationEventComponent> outEvents, ref BlobArray<AnimationEventBlob> events, float2 timeSpan)
	{
		for (var j = 0; j < events.Length && timeSpan.y >= events[j].time; ++j)
		{
			ref var ae = ref events[j];
			if (ae.time > timeSpan.x)
			{
				var evt = new AnimationEventComponent(ref ae);
				outEvents.Add(evt);
